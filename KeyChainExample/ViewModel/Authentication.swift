...
class Authentication: ObservableObject {
    ...
    enum AuthenticationError: Error, LocalizedError, Identifiable {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidCredentials:
                return NSLocalizedString("Enter your email or password are incorrect. Please try again", comment: "")
            case .deniedAccess:
...
//        let credentials: Credentials? = Credentials(email: "anthing", password: "password")
//        let credentials: Credentials? = nil
        let credentials = KeychainStorage.getCredentials()
        
        guard let crednetials = credentials else {
                ...
                context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: "Need to access credentials.") { success, error in
                    DispatchQueue.main.async {
                        if error != nil {
                            ...
                            completion(.success(crednetials))
                        }
                    }
                }
            }
